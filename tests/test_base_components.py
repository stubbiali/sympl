...
class InputTestBase:
    ...
    def test_collects_independent_wildcard_dims(self):
        ...
        state = {
            "time": timedelta(0),
            "input1": DataArray(
                np.zeros([4]), dims=["dim1"], attrs={"units": "m"},
            ),
            "input2": DataArray(
                np.zeros([3]), dims=["dim2"], attrs={"units": "m"},
            ),
        }
        self.call_component(component, state)
    ...
    def test_accepts_when_input_swapped_dims(self):
        input_properties = {
            "input1": {"dims": ["dim1", "dim2"], "units": "m",}
        }
        component = self.get_component(input_properties=input_properties)
        state = {
            "time": timedelta(0),
            "input1": DataArray(
                np.zeros([3, 4]), dims=["dim2", "dim1"], attrs={"units": "m"},
            ),
        }
        self.call_component(component, state)
    ...
    def test_input_converts_units(self):
        ...
        assert isinstance(component.state_given["input1"], np.ndarray)
        assert np.all(
            component.state_given["input1"] == np.ones([10]) * 1000.0
        )

    def test_input_converts_temperature_units(self):
        input_properties = {"input1": {"dims": ["dim1"], "units": "degK"}}
        component = self.get_component(input_properties=input_properties)
        state = {
            "time": timedelta(0),
            "input1": DataArray(
                np.ones([10]), dims=["dim1"], attrs={"units": "degC"}
            ),
        }
        self.call_component(component, state)
        assert len(component.state_given) == 2
        ...
        assert isinstance(component.state_given["input1"], np.ndarray)
        assert np.all(
            component.state_given["input1"] == np.ones([10]) * 274.15
        )

    def test_input_collects_one_dimension(self):
        input_properties = {"input1": {"dims": ["*"], "units": "m"}}
        component = self.get_component(input_properties=input_properties)
        state = {
            "time": timedelta(0),
            "input1": DataArray(
    ...
    def test_input_is_aliased(self):
        input_properties = {
            "input1": {"dims": ["dim1"], "units": "m", "alias": "in1",}
        }
        component = self.get_component(input_properties=input_properties)
        state = {
            "time": timedelta(0),
...
class DiagnosticTestBase:
    ...
    def test_diagnostic_raises_when_units_incompatible_with_input(self):
        input_properties = {"diag1": {"units": "km", "dims": ["dim1", "dim2"]}}
        diagnostic_properties = {
    ...
    def test_diagnostic_requires_correct_number_of_dims(self):
        input_properties = {"input1": {"units": "m", "dims": ["dim1", "dim2"]}}
        diagnostic_properties = {
        ...
        state = {
            "time": timedelta(0),
            "input1": DataArray(
                np.ones([10, 2]), dims=["dim1", "dim2"], attrs={"units": "m"}
            ),
        }
    ...
    def test_diagnostics_no_transformations(self):
        diagnostic_properties = {"output1": {"dims": ["dim1"], "units": "m"}}
        diagnostic_output = {
            "output1": np.ones([10]),
        }
        component = self.get_component(
    ...
    def test_diagnostics_with_alias(self):
        diagnostic_properties = {
            "output1": {"dims": ["dim1"], "units": "m", "alias": "out1",}
        }
        diagnostic_output = {
            "out1": np.ones([10]),
        }
    ...
    def test_diagnostics_with_dims_from_input(self):
        input_properties = {"output1": {"dims": ["dim1"], "units": "m",}}
        diagnostic_properties = {"output1": {"units": "m",}}
        diagnostic_output = {
            "output1": np.ones([10]),
        }
        component = self.get_component(
    ...
    def test_raises_when_diagnostic_not_given(self):
        diagnostic_properties = {"diag1": {"dims": ["dims1"], "units": "m",}}
        diagnostic_output = {}
...
class PrognosticTests(unittest.TestCase, InputTestBase):
    ...
    def test_raises_when_tendency_not_given(self):
        input_properties = {}
        diagnostic_properties = {}
        tendency_properties = {"tend1": {"dims": ["dims1"], "units": "m",}}
        diagnostic_output = {}
        tendency_output = {}
        prognostic = self.component_class(
            input_properties,
    ...
    def test_tendencies_with_alias(self):
        ...
        diagnostic_properties = {}
        tendency_properties = {
            "output1": {"dims": ["dim1"], "units": "m/s", "alias": "out1",}
        }
        diagnostic_output = {}
        tendency_output = {
            "out1": np.ones([10]),
    ...
    def test_tendencies_with_dims_from_input(self):
        input_properties = {"output1": {"dims": ["dim1"], "units": "m",}}
        diagnostic_properties = {}
        tendency_properties = {"output1": {"units": "m/s",}}
        diagnostic_output = {}
        tendency_output = {
            "output1": np.ones([10]),
        }
    ...
    def test_tendencies_in_diagnostics_one_tendency(self):
        ...
        assert np.all(diagnostics[tendency_name].values == 20.0)
...
class ImplicitTests(unittest.TestCase, InputTestBase, DiagnosticTestBase):
    ...
    def test_raises_when_output_not_given(self):
        input_properties = {}
        diagnostic_properties = {}
        output_properties = {"output1": {"dims": ["dims1"], "units": "m",}}
        diagnostic_output = {}
        state_output = {}
        implicit = self.component_class(
            input_properties,
    ...
    def test_output_no_transformations(self):
        input_properties = {}
        diagnostic_properties = {}
        output_properties = {"output1": {"dims": ["dim1"], "units": "m/s"}}
        diagnostic_output = {}
        output_state = {
            "output1": np.ones([10]),
        }
    ...
    def test_output_with_dims_from_input(self):
        input_properties = {"output1": {"dims": ["dim1"], "units": "m",}}
        diagnostic_properties = {}
        output_properties = {"output1": {"units": "m",}}
        diagnostic_output = {}
        output_state = {
            "output1": np.ones([10]),
        }
    ...
    def test_tendencies_in_diagnostics_one_tendency(self):
        ...
        assert (
            ...
            in implicit.diagnostic_properties.keys()
        )
        assert (
            "output1" in input_properties.keys()
        ), "Stepper needs original value to calculate tendency"
        ...
        assert len(diagnostics["output1_tendency_from_MockStepper"].dims) == 1
        assert "dim1" in diagnostics["output1_tendency_from_MockStepper"].dims
        assert (
            diagnostics["output1_tendency_from_MockStepper"].attrs["units"]
            == "m s^-1"
        )
        assert np.all(
            diagnostics["output1_tendency_from_MockStepper"].values == 2.0
        )
    ...
    def test_tendencies_in_diagnostics_one_tendency_mismatched_units(self):
        input_properties = {"output1": {"dims": ["dim1"], "units": "km"}}
        diagnostic_properties = {}
        output_properties = {"output1": {"dims": ["dim1"], "units": "m"}}
        diagnostic_output = {}
        output_state = {
    ...
    def test_tendencies_in_diagnostics_one_tendency_mismatched_dims(self):
        input_properties = {"output1": {"dims": ["dim1"], "units": "m"}}
        diagnostic_properties = {}
        output_properties = {"output1": {"dims": ["dim2"], "units": "m"}}
        diagnostic_output = {}
        output_state = {
            "output1": np.ones([10]) * 20.0,
        }
    ...
    def test_tendencies_in_diagnostics_one_tendency_with_component_name(self):
        ...
        state = {
            "time": timedelta(0),
            "output1": DataArray(
                np.ones([10]) * 2.0, dims=["dim1"], attrs={"units": "m"}
            ),
        }
        diagnostics, _ = implicit(state, timedelta(seconds=5))
        ...
        assert np.all(
            diagnostics["output1_tendency_from_component"].values == 1.0
        )
