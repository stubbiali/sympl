...
class PrognosticBase(object):
    ...
    def test_given_tendency_not_modified_with_two_components(self):
        input_properties = {}
        diagnostic_properties = {}
        tendency_properties = {"tend1": {"dims": ["dim1"], "units": "m/s",}}
        diagnostic_output = {}
        tendency_output_1 = {"tend1": np.ones([10]) * 5.0}
        ...
        tendency_output_2 = {"tend1": np.ones([10]) * 5.0}
        ...
        assert np.all(tendency_output_1["tend1"] == 5.0)
        assert np.all(tendency_output_2["tend1"] == 5.0)
    ...
    def test_input_state_not_modified_with_two_components(self):
        ...
        assert np.all(state["tend1"].values == 1.0)
    ...
    def test_tendencies_in_diagnostics_one_tendency(self):
        ...
        state = {
            "time": timedelta(0),
            "output1": DataArray(
                np.ones([10]) * 10.0, dims=["dim1"], attrs={"units": "m"}
            ),
        }
        diagnostics, _ = stepper(state, timedelta(seconds=5))
        ...
        assert len(diagnostics[tendency_name].dims) == 1
        assert "dim1" in diagnostics[tendency_name].dims
        assert units_are_compatible(
            diagnostics[tendency_name].attrs["units"], "m s^-1"
        )
        assert np.allclose(diagnostics[tendency_name].values, 2.0)
    ...
    def test_tendencies_in_diagnostics_one_tendency_with_component_name(self):
        ...
        assert np.allclose(
            diagnostics["output1_tendency_from_component"].values, 2.0
        )
    ...
    def test_copies_untouched_quantities(self):
        ...
        untouched_quantity = DataArray(
            np.ones([10]) * 10.0, dims=["dim1"], attrs={"units": "J"}
        )
        state = {
            "time": timedelta(0),
            "output1": DataArray(
        ...
        assert np.allclose(new_state["input1"].values, 10.0)
    ...
    def test_stepper_gives_diagnostic_quantity(self):
        input_properties = {}
        diagnostic_properties = {"diag1": {"dims": ["dim2"], "units": "",}}
        tendency_properties = {}
        diagnostic_output = {}
        tendency_output = {}
        prognostic = self.prognostic_class(
...
class TimesteppingBase(object):
    ...
    def test_unused_quantities_carried_over(self):
        state = {"time": timedelta(0), "air_temperature": 273.0}
        ...
        assert state == {"time": timedelta(0), "air_temperature": 273.0}
        assert new_state == {"time": timedelta(0), "air_temperature": 273.0}
    ...
    def test_float_no_change_one_step(self, mock_prognostic_call):
        mock_prognostic_call.return_value = ({"air_temperature": 0.0}, {})
        state = {"time": timedelta(0), "air_temperature": 273.0}
        timestep = timedelta(seconds=1.0)
        time_stepper = self.timestepper_class(MockEmptyTendencyComponent())
        diagnostics, new_state = time_stepper.__call__(state, timestep)
    ...
    def test_float_one_step(self, mock_prognostic_call):
        mock_prognostic_call.return_value = ({"air_temperature": 1.0}, {})
        state = {"time": timedelta(0), "air_temperature": 273.0}
        timestep = timedelta(seconds=1.0)
        time_stepper = self.timestepper_class(MockEmptyTendencyComponent())
        diagnostics, new_state = time_stepper.__call__(state, timestep)
        assert state == {"time": timedelta(0), "air_temperature": 273.0}
        assert new_state == {"time": timedelta(0), "air_temperature": 274.0}

    @mock.patch.object(MockEmptyTendencyComponent, "__call__")
    def test_float_one_step_with_units(self, mock_prognostic_call):
        mock_prognostic_call.return_value = (
            {"eastward_wind": DataArray(0.02, attrs={"units": "km/s^2"})},
            {},
        )
        state = {
            "time": timedelta(0),
            "eastward_wind": DataArray(1.0, attrs={"units": "m/s"}),
        }
        timestep = timedelta(seconds=1.0)
        time_stepper = self.timestepper_class(MockEmptyTendencyComponent())
        diagnostics, new_state = time_stepper.__call__(state, timestep)
        ...
        assert np.allclose(new_state["eastward_wind"].values, 21.0)
    ...
    def test_float_three_steps(self, mock_prognostic_call):
        ...
        assert state == {"time": timedelta(0), "air_temperature": 274.0}
        assert new_state == {"time": timedelta(0), "air_temperature": 275.0}
        state = new_state
        diagnostics, new_state = time_stepper.__call__(state, timestep)
        assert state == {"time": timedelta(0), "air_temperature": 275.0}
        assert new_state == {"time": timedelta(0), "air_temperature": 276.0}
    ...
    def test_array_no_change_one_step(self, mock_prognostic_call):
        ...
        state = {
            "time": timedelta(0),
            "air_temperature": np.ones((3, 3)) * 273.0,
        }
        timestep = timedelta(seconds=1.0)
        time_stepper = self.timestepper_class(MockEmptyTendencyComponent())
        diagnostics, new_state = time_stepper.__call__(state, timestep)
        assert same_list(state.keys(), ["time", "air_temperature"])
        assert (state["air_temperature"] == np.ones((3, 3)) * 273.0).all()
        assert same_list(new_state.keys(), ["time", "air_temperature"])
        assert (new_state["air_temperature"] == np.ones((3, 3)) * 273.0).all()

    @mock.patch.object(MockEmptyTendencyComponent, "__call__")
    def test_array_no_change_three_steps(self, mock_prognostic_call):
        mock_prognostic_call.return_value = (
            {"air_temperature": np.ones((3, 3)) * 0.0},
            {},
        )
        state = {
            "time": timedelta(0),
    ...
    def test_array_one_step(self, mock_prognostic_call):
        mock_prognostic_call.return_value = (
            {"air_temperature": np.ones((3, 3)) * 1.0},
            {},
        )
        state = {
            "time": timedelta(0),
        ...
        assert (state["air_temperature"] == np.ones((3, 3)) * 273.0).all()
        assert same_list(new_state.keys(), ["time", "air_temperature"])
        assert (new_state["air_temperature"] == np.ones((3, 3)) * 274.0).all()
    ...
    def test_array_three_steps(self, mock_prognostic_call):
        ...
        diagnostics, new_state = time_stepper.__call__(new_state, timestep)
        assert same_list(state.keys(), ["time", "air_temperature"])
        assert (state["air_temperature"] == np.ones((3, 3)) * 274.0).all()
        assert same_list(new_state.keys(), ["time", "air_temperature"])
        assert (new_state["air_temperature"] == np.ones((3, 3)) * 275.0).all()
        state = new_state
        diagnostics, new_state = time_stepper.__call__(state, timestep)
        assert same_list(state.keys(), ["time", "air_temperature"])
        assert (state["air_temperature"] == np.ones((3, 3)) * 275.0).all()
        assert same_list(new_state.keys(), ["time", "air_temperature"])
        assert (new_state["air_temperature"] == np.ones((3, 3)) * 276.0).all()
    ...
    def test_dataarray_no_change_one_step(self, mock_prognostic_call):
        ...
        state = {
            "time": timedelta(0),
            "air_temperature": DataArray(
                np.ones((3, 3)) * 273.0, attrs={"units": "K"}
            ),
        }
        timestep = timedelta(seconds=1.0)
        time_stepper = self.timestepper_class(MockEmptyTendencyComponent())
        ...
        assert same_list(state.keys(), ["time", "air_temperature"])
        assert (
            state["air_temperature"].values == np.ones((3, 3)) * 273.0
        ).all()
        assert len(state["air_temperature"].attrs) == 1
        assert state["air_temperature"].attrs["units"] == "K"
        assert same_list(new_state.keys(), ["time", "air_temperature"])
        assert (
            new_state["air_temperature"].values == np.ones((3, 3)) * 273.0
        ).all()
    ...
    def test_array_four_steps(self, mock_prognostic_call):
        ...
        diagnostics, new_state = time_stepper.__call__(state, timestep)
        assert same_list(state.keys(), ["time", "air_temperature"])
        assert (state["air_temperature"] == np.ones((3, 3)) * 276.0).all()
        assert same_list(new_state.keys(), ["time", "air_temperature"])
        assert (new_state["air_temperature"] == np.ones((3, 3)) * 277.0).all()
...
class TestAdamsBashforthFirstOrderImplicitPrognostic(
    TimesteppingBase, ImplicitPrognosticBase
):
    def timestepper_class(self, *args, **kwargs):
        kwargs["order"] = 1
        return AdamsBashforth(*args, **kwargs)
...
class TestSSPRungeKuttaTwoStepImplicitPrognostic(
    TimesteppingBase, ImplicitPrognosticBase
):
    def timestepper_class(self, *args, **kwargs):
        kwargs["stages"] = 2
...
class TestSSPRungeKuttaThreeStepImplicitPrognostic(
    TimesteppingBase, ImplicitPrognosticBase
):
    def timestepper_class(self, *args, **kwargs):
        kwargs["stages"] = 3
...
class TestAdamsBashforthSecondOrderImplicitPrognostic(
    TimesteppingBase, ImplicitPrognosticBase
):
    def timestepper_class(self, *args, **kwargs):
        kwargs["order"] = 2
...
class TestAdamsBashforthThirdOrderImplicitPrognostic(
    TimesteppingBase, ImplicitPrognosticBase
):
    def timestepper_class(self, *args, **kwargs):
        kwargs["order"] = 3
...
class TestAdamsBashforthFourthOrderImplicitPrognostic(
    TimesteppingBase, ImplicitPrognosticBase
):
    def timestepper_class(self, *args, **kwargs):
        kwargs["order"] = 4
...
def test_leapfrog_float_two_steps_filtered(mock_prognostic_call):
    ...
    state = {"time": timedelta(0), "air_temperature": 273.0}
    timestep = timedelta(seconds=1.0)
    time_stepper = Leapfrog(
        MockEmptyTendencyComponent(), asselin_strength=0.5, alpha=1.0
    )
    diagnostics, new_state = time_stepper.__call__(state, timestep)
    assert state == {"time": timedelta(0), "air_temperature": 273.0}
    assert new_state == {"time": timedelta(0), "air_temperature": 273.0}
    state = new_state
    mock_prognostic_call.return_value = ({"air_temperature": 2.0}, {})
    diagnostics, new_state = time_stepper.__call__(state, timestep)
    # Asselin filter modifies the current state
    assert state == {"time": timedelta(0), "air_temperature": 274.0}
    assert new_state == {"time": timedelta(0), "air_temperature": 277.0}
...
def test_leapfrog_requires_same_timestep(mock_prognostic_call):
    ...
    state = {"time": timedelta(0), "air_temperature": 273.0}
    time_stepper = Leapfrog(
        [MockEmptyTendencyComponent()], asselin_strength=0.5
    )
    diagnostics, state = time_stepper.__call__(state, timedelta(seconds=1.0))
    try:
        time_stepper.__call__(state, timedelta(seconds=2.0))
    except ValueError:
        pass
...
def test_adams_bashforth_requires_same_timestep(mock_prognostic_call):
    ...
    state = time_stepper.__call__(state, timedelta(seconds=1.0))
    try:
        time_stepper.__call__(state, timedelta(seconds=2.0))
    except ValueError:
        pass
...
def test_leapfrog_array_two_steps_filtered(mock_prognostic_call):
    ...
    state = {"time": timedelta(0), "air_temperature": np.ones((3, 3)) * 273.0}
    timestep = timedelta(seconds=1.0)
    time_stepper = Leapfrog(
        MockEmptyTendencyComponent(), asselin_strength=0.5, alpha=1.0
    )
    ...
    state = new_state
    mock_prognostic_call.return_value = (
        {"air_temperature": np.ones((3, 3)) * 2.0},
        {},
    )
    diagnostics, new_state = time_stepper.__call__(state, timestep)
    # Asselin filter modifies the current state
...
def test_leapfrog_array_two_steps_filtered_williams(mock_prognostic_call):
    ...
    timestep = timedelta(seconds=1.0)
    time_stepper = Leapfrog(
        MockEmptyTendencyComponent(), asselin_strength=0.5, alpha=0.5
    )
    diagnostics, new_state = time_stepper.__call__(state, timestep)
    assert same_list(state.keys(), ["time", "air_temperature"])
    assert (state["air_temperature"] == np.ones((3, 3)) * 273.0).all()
